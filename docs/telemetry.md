...
## Correlation IDs and Sentry links

Every emitted security event carries a `correlation_id` (32 lowercase hex chars,
generated by `newCorrelationId()` in `src/lib/telemetry.ts`). It is used as the
Sentry `event_id`, added as a Sentry tag, and persisted where the event has a
durable record:

| Table | Column | Written by |
| --- | --- | --- |
| `security_export_audit` | `correlation_id` | `recordExportAudit` in `src/lib/security.functions.ts` |
| `security_export_jobs` | `correlation_id` | `startSecurityExportJob` |
| `security_sync_attempts` | `correlation_id` | `/api/public/security-sync` |

The admin export-audit panel and the export-jobs page render each ID as a direct
Sentry search link (`correlation_id:<id>`), built by `src/lib/sentry-link.ts`.
Configure `VITE_SENTRY_ORG` and `VITE_SENTRY_PROJECT` to enable the links; when
unset, the ID renders as plain text.

## Async export job events

| Event | Severity | Attributes |
| --- | --- | --- |
| `security.export.denied` | warning | `dataset`, `user_id`, `mode` (`async` / `download`) |
| `security.export.completed` | info | `dataset`, `mode`, `job_id`, `rows`, `bytes`, `user_id` |
| `security.export.failed` | error | `dataset`, `mode`, `job_id`, `error` |
| `security.export.downloaded` | info | `dataset`, `job_id`, `user_id` |

## Scan-diff backfill

The scan diff page tracks a watermark (`src/lib/scan-watermark.ts`) of the newest
rendered scan timestamp. On realtime `SUBSCRIBED`, tab focus, network recovery,
and a 60s interval it calls `getLatestScanAt`; when the server is ahead of the
watermark, a scan completed while the socket was down and the diff refetches.
